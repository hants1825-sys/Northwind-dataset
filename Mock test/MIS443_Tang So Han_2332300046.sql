...
STUDENT ID : Tang So Han
FULL NAME  : 2332300046
GITHUB URL :
===============================================================================
*/
...
(10 marks)
*/
select b.branch_id, b.branch_name, b.city, round(avg(a.balance),2) as average_balance
from branches b
...
HAVING COUNT(t.transaction_id) = (
    SELECT MAX(cnt)
    FROM (
        SELECT COUNT(t2.transaction_id) AS cnt
        FROM   accounts a2
...
FROM   branch_tx
WHERE  total_transactions = (SELECT MAX(total_transactions) FROM branch_tx);
-- Expected result:
-- 1 | Main      | 4
-- 8 | South Bay | 4
